#pragma config(Sensor, S1, infra_pred, sensorEV3_IRSensor)
#pragma config(Sensor, S2, infra_levy, sensorEV3_IRSensor)
#pragma config(Sensor, S3, infra_pravy, sensorEV3_IRSensor)
...
//! WTF all global variables should be prefixed with g_ (or labeled in some other similar fashion, so that you know what variables are global just by looking at them)
...
int e1 = 0;
int e2 = 0;
int trueorfalse = 0; //! WTF I know that naming stuff can be hard, but seriously?!

// vsechny rozmery hriste a delky jizdy jsou v mm
// const int delkaStart = 200; // delka startovaci oblasti
const int delka1 = 470; // delka prvniho useku "S" - kolik robot pojede ze startovaci pozice //W
...
task senzory()
{
    g_distPredni = getIRDistance(infra_pred);
    g_distLevy = getIRDistance(infra_levy);
    g_distPravy = getIRDistance(infra_pravy);
    delay(2);
}

// zaporna rychlost - jede vzad; zaporna draha - jede porad VPRED !!
void jizda(int draha, int rychlost)
{ // jed rovne danou drahu danou rychlosti
...
void oblouk(int polomer, int stupne, int rychlost)
{ // projed oblouk o danem polomeru, danem poctu stupnu a dane rychlosti
  // polomer > 0 - zataci vpravo; polomer < 0 - zataci vlevo;
    bool zataci_vlevo = false;

    if (polomer < 0) {
        zataci_vlevo = polomer < 0;
...
void jizda1()
{ // hleda konec S
    float distance2 = 300; //! WTF
    // while (true) {
    do {
        distance2 = getIRDistance(infra_levy);
...
float hledaniMedveda() //! WTF why return float?
{
    float distLastPredni = 300; //! WTF? why this arbitrary value
    ...
    do {
        ...
        delay(20); //! WTF replace this with actual reading, don't expect the reading to run exactly once, you will miss the bear 90% of the time

        displayBigTextLine(1, "P %i", distLastPredni);
        displayBigTextLine(3, "L %i", distLastLevy);
        displayBigTextLine(5, "R %i", distLastPravy);

    } while ((g_distLevy - distLastLevy < 10) && (g_distPravy - distLastPravy < 10)); //! WTF how can you know it changed?
...
void hledaniMedveda1()
{
    ...
    do {
        distance1 = getIRDistance(infra_pred);
...
void jizdaVpred()
{
    ...
    do {
        distance2 = getIRDistance(infra_pravy); // levy infra
        displayBigTextLine(1, "A %i", distance2);
        setMotorSpeed(motorL, speed);
        setMotorSpeed(motorR, speed);
    } while (distance2 > 50);
...
float medved1()
{
    float d = getIRDistance(infra_pred);
    displayBigTextLine(1, "A %i", d); //! WTF use constants
    return d;
}

float medved2()
{
    float d = getIRDistance(infra_levy);
    displayBigTextLine(3, "B %i", d); //! WTF use constants
    return d;
}

float medved3()
{
    float d = getIRDistance(infra_pravy);
    displayBigTextLine(5, "C %i", d); //! WTF use constants
    return d;
}
